...
//| Title=Signals of oscillator 'MACD_KK'                            |
//| Type=SignalAdvanced                                              |
//| Name=MACD_KK                                                     |
//| ShortName=MACDKK                                                 |
//| Class=CSignalMACD                                                |
...
protected:
   ...
   double             m_take_profit;        // level to place a stop loss order relative to the open price
...
//--- check of exceeding the threshold value
   if(-m_direction>=m_threshold_open)
     {
      sl   =m_symbol.NormalizePrice(price+m_stop_loss);//unit
      //Debug(); printf(__FUNCTION__+": sl setup ",DoubleToString(sl,Digits()));
      //--- there's a signal
     result=true;
      //--- try to get the levels of opening
